...
const nameCharLimit = 400 // character limit for names
...
//   profile_image?: string // Character limit
...
//   name?: string // Character limit
//   profile_image?: string // Size limit
...
export function checkNewUser(incoming: UserModels.NewUserModel): Result {
  ...
  if (incoming.profile_image) {
    // is a string
    if (typeof incoming.profile_image !== 'string') {
      result.errors.push(
        `'profile_image' property should be of type 'string', instead recieved '${typeof incoming.profile_image}'`
      )
    }
...
export function checkNewRock(incoming: RockModels.NewRockModel): Result {
  ...
  if (incoming.image) {
    ...
    if (typeof incoming.image !== 'string') {
      result.errors.push(
        `'image' property should be of type 'string', instead recieved '${typeof incoming.image}'`
      )
    }
  ...
  // is contained in list of valid weight divisions
  let validDivs = ''
